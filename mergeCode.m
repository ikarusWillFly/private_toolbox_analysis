...
if ~isdir(dataPath2), mkdir(dataPath2); warning('creating a new directory %s',dataPath2), end
...
for n_files = 1: numel(folders)
    ...
    if ~isdir(folder2), mkdir(folder2);
        warning('creating a new directory %s',folder2)
    end
        ...
        switch  strrep(type,'safe','')
            case 'merge'
                if date(1) > date(2)
                    ...
                    warning('overwriting %s to %s',mFile1,mFile2)
                end
            case 'copy'
                if strfind(type,'safe'), copyfile(mFile2,[folder2,'OLD_',mFile]), end
                copyfile(mFile1,mFile2)
